...
```{r, echo = FALSE, out.width = "80%"}
include_graphics("out-of-date2.png")
```
...
```{r, echo = FALSE, out.width = "40%"}
include_graphics("data_preview2.png")
```
...
The `charge_balance` values will be highlighted in green (value ranges from -10% to 10%) or red (values < -10% or > 10%) to indicate possibly problematic samples.
You can chose whether to highlight EMS-calculated charge balances or rems2aquachem-calculated balances by selecting the appropriate option under "Charge Balance to use".
...
```{r, echo = FALSE, out.width = "80%"}
include_graphics("water_quality.png")
```
